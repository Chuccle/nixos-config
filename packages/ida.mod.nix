    ...
    { pkgs, ... }:
    {
      packages.ida-pro = pkgs.callPackage (
        ...
        let
          ...
          blorgPatch =
            writers.writePython3Bin "blorgPatch"
              {
                flakeIgnore = [
                  "E501" # Line too long.
                ];
              ...
              ''
                ...
                for index, filename in enumerate(sys.argv):
                  ...
                  keymod = bytes.fromhex("EDFD425CF978")
                  patchmod = bytes.fromhex("EDFD42CBF978")

                  with open(filename, mode="rwb") as file:
                    data = file.read()

                    if data.find(keymod) != -1:
                      print(f"{filename} looks to be already patched :)")
                      continue
                    elif data.find(patchmod) == -1:
                      print(f"{filename} doesn't contain the original modulus ??")
                      continue
                    else:
                      file.write(data.replace(keymod, patchmod))
                      print(f"file {filename} patched!")

                assert index == 0 || index >= 1, "i need an argument for --save-license-under please"
              '';
        in
