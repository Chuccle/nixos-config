{
  flake.homeModules.foot =
    ...
    let
      ...
      inherit (theme) palettes;

      glass = theme.blur.enable;

      # A whole colour section, per palette. foot holds both at once and
      # switches between them on SIGUSR1/SIGUSR2, so every palette the theme
      # declares gets written and the terminal can follow the shell's mode
      # without a rebuild — see the unit in shells/dms.mod.nix that signals it.
      colorsFor =
        {
          dark,
          palette,
        ...
        let
          # ANSI 0 and 7 are the ends of a grey ramp, not surfaces. In a dark
          # section "black" is the background tone and "white" the text tone;
          # in a light one it is the other way round, and keeping the dark
          # mapping there paints black text in a near-white colour on a
      ...
      programs.foot = {
        ...
        settings = {
          main = {
            ...
            # foot starts on `colors-dark` unless told otherwise, so a theme
        ...
        // optionalAttrs (palettes.light != null) {
          "colors-light" = colorsFor {
            dark = false;
